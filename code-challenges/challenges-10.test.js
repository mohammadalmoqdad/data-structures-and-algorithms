...
const createServer = () => {
  ...
  let aboutme = 'My name is mohammad, I am in my way to be great web developer ISA'
  app.get('/aboutme', (request, response) => {
    response.status(200).send(aboutme);
  })
  let favoritefoods = ['mansaf', 'makmora', 'maqloba'];
...
const count = (target, input) => {
  // Solution code here...
  var newarr;
  let outArrTarget = input.reduce((acc,val,idx) => {
    acc=acc.concat(val);
    return acc;
  },[]);
  let counter=outArrTarget.filter(val=>{
    if(val==target){
      return true;
    }
  })
  ...
  for(let i = 0; i < input.length; i++){
    ...
    for(let j = 0; j < input[i].length; j++){
      if(input[i][j] % 5 == 0 && typeof input[i][j] === 'number'){
        arrOfNums.push(Math.pow(2, input[i][j]));
      }
    }
    arrOAllNums.push(arrOfNums);
  }
  return arrOAllNums;
};
